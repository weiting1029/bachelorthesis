...
rate_table_p50 = np.empty((len(can_s), len(can_rho)))
...
def main():
    ...
    for i in range(len(can_s)):
        for j in range(len(can_rho)):
            rate_table_p50[i, j] = func.av_rates_ldpe(seed, KK, alpha, n, 50, can_s[i], can_rho[j], pool)
            print(rate_table_p50[i, j])
        # s_rate_table_p100[i] = func.av_rates_ldpe(seed, 100, alpha, n, 100, can_s[i], 0.6, pool)
        # s_rate_table_p150[i] = f unc.av_rates_ldpe(seed, 100, alpha, n, 150, can_s[i], 0.6, pool)
    pool.close()
    pool.join()
